...
class Chromosome():
    ...
    def calculate_sumsq_function(self):
        """ Calculate sumsquare function with x and y extracted from chromosome.

        :return: sumsquare function output
        :rtype: float
        """
        sumsq_x = self.get_x()
        sumsq_y = self.get_y()    
        return (1 * sumsq_x**2) + (2 * sumsq_y**2)

    def calculate_fitness(self, base=21):
        """ Calculate fitness by subtracting bohachevsky function value from 21,
        since maximum value of bohachevsky function is 20, our worst chromosome
        ...
        :rtype: float
        """
        return base - self.calculate_sumsq_function()
